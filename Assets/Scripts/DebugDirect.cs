...
public class DebugDirect : MonoBehaviour
{
    ...
    void AddDirect()
    {
        ...
        if (field != null)
        {
            ...
            if (items != null && items.Count > 0)
            {
                ...
                Debug.Log($"아이템: {item.name}, 타입: {itemType}");
        ...
        if (recipeField != null)
        {
            ...
            if (recipe != null)
            {
                ...
                if (ingredientsProp != null)
                {
                    ...
                    Debug.Log($"현재 레시피 재료 개수: {ingredients.Count}");
                    
                    for (int i = 0; i < ingredients.Count; i++)
                    {
                        Debug.Log($"재료 {i}: {ingredients[i].cropType} ({ingredients[i].currentAmount}/{ingredients[i].requiredAmount})");
                    }
                    ...
                    foreach (var ing in ingredients)
                    {
                        ...
                        {
                            ing.currentAmount++;
                            Debug.Log($"성공! {itemType}: {ing.currentAmount}/{ing.requiredAmount}");
                    ...
                    Debug.Log($"{itemType}는 필요 없거나 이미 충분함");
                }
            }
        }
    }
